# Connection Framework (Layer 6)

Maps for connecting a concept to broader patterns, systems, and interview relevance. Use these in Layer 6 (CONNECT). Don't force connections — only mention them when they genuinely reinforce understanding or when the learner asks "where is this used?"
...
| Tries | Autocomplete, DNS lookup, IP routing tables, spell checkers, prefix search |
...
| Binary search | Database index lookups, git bisect, rotated-array search, finding boundaries in sorted data |
...
| Observability | Metrics, logging (ELK), tracing (Jaeger/X-Ray), alerting, SLOs/SLIs |
...
**Rule of use:** Don't force connections. Only mention them when they genuinely reinforce understanding or when the learner asks "where is this used?" One strong connection beats three weak ones.
